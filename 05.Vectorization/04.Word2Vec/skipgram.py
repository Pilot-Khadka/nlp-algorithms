...
class Skipgram:
    ...
    def _build_vocab(self):
        ...
        word_freq = np.array([self.word_counts[word] for word in self.vocab], dtype=np.float32)
    ...
    def _create_samples_table(self, table_size=1e6):
        ...
        sample_table = np.concatenate([np.full(freq, idx) for idx, freq in zip(word_indices, word_probs)])
    ...
    def _build_dataset(self, batch_size=64):
        ...
        for i in range(total_words):
            ...
            if len(window) == 2*self.window_size+1:
                ...
                context_words = [window[j] for j in range(len(window)) if j != center_idx]
                ...
                for _ in range(self.neg_samples):
                    negative_idx = np.random.choice(self.negative_samples_table, size=1)
                    dataset.append((center_idx, int(negative_idx), 0))
            print(f"\rProcessing {i+1}/{total_words} words", end="\r", flush=True)
