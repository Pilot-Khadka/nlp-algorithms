...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x = x + self.attention(self.norm1(x), self.norm2(x), self.norm1(x), mask)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, x, encoder_out, source_mask=None, target_mask=None):
        # query, key, value
        # residual connections
        x = x + self.attention(self.norm1(x), self.norm1(x), self.norm1(x), target_mask)

        # cross-attention uses key from encoder
        x = x + self.cross_attention(
            self.norm2(x), self.norm2(encoder_out), self.norm2(encoder_out), source_mask
        )
